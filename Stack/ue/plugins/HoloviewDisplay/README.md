...
requirements. There is no dual-copy gate and no mount to compare: the
experience consumes this plugin where it lives, through its project
descriptor's additional plugin directories, so exactly one tree exists and
drift between copies is not possible rather than merely detected. Binaries
land in the consuming project, leaving this repository clean.
