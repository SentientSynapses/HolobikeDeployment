...
`Stack/` is the per-repository chapter of the Spec: the software this
repository composes. The first level is a short HoloBike software domain,
...
trees — and they hold no executable adapter code. The Spec binds; workflows
execute. An adapter implementation lands under `Development/Assembly/`, bound
...
versions are compatible, which gates admit a release — it is a Spec kind and
lives beside `Stack/`, not inside it. What stays here is the contract for
driving one repository and what that repository exposes.
